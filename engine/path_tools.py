...
def path_to(actor: tcod.ecs.Entity, dest: Position) -> list[Position]:
    ...
    for other in actor.registry.Q.all_of(tags=[IsActor], relations=[(InMap, map_)]).none_of(tags=[IsDead]):
        ...
        if cost[other_pos.packed]:
            ...
            cost[other_pos.packed] += PATH_COST_INCREASE
    ...
    # Starting position.
    pathfinder.add_root(actor.components[Position].packed)

    # Compute path to destination and remove starting node.
    path: list[list[int]] = pathfinder.path_to(dest.packed)[1:].tolist()
